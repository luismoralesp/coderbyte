...
The plus (+) character represents a single alphabetic character, the asterisk (*) represents a sequence
 of the same character of length 3 unless it is followed by {N} which represents how many characters should
  appear in the sequence where N will be at least 1. Your goal is to determine if the second string exactly 
...
def WildcardCharacters(str): 
    ...
    exp = exp.replace('*', '.{3}')
    exp = '^%s$' % (exp,)
    return "true" if re.match(exp, str) else "false"
